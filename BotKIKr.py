...
class BotKIKr:
    ...
    def kospi_filter(self):
        ...
        return self.get_caution_code_list(kp['단축코드'].to_list(), True)
    
    def kosdaq_filter(self):
        ...
        return self.get_caution_code_list(kd['단축코드'].to_list(), True)
    
    def get_condition_code_list(self):

        ft_kp = self.kospi_filter()
        ...
        for symbol in symbol_list:
            ...
            symbol_data = self.fetch_ohlcv_domestic(symbol, 'D', tn_3m.strftime('%Y%m%d'), tn.strftime('%Y%m%d'))['output2']
            ...
            df = gen_soar_df(_df).tail(1)
            cls_v = df['close'].iloc[-1]
            cls_40_x_v = df['close_40_max'].iloc[-1]
            vol_10_m_v = df['volume_10_mean'].iloc[-1]
            vol_v = df['volume'].iloc[-1]
            cls_10_h_v = df['close_10_hgt'].iloc[-1]
            
            if \
            (1000 < cls_v) and \
            (cls_40_x_v < cls_v) and \
            (vol_10_m_v * 2.5 < vol_v) and \
            (cls_10_h_v < 150) \
            :
